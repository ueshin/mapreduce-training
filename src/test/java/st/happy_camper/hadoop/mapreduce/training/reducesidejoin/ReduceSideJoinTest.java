 ...
 * @author ueshin
 */
public class ReduceSideJoinTest extends ClusterMapReduceTestCase {
     ...
     */
    @Test
    public void testReduceSideJoin() throws Exception {
        FileSystem fs = getFileSystem();
        Path joinFilePath = new Path("links.txt");
        fs.copyFromLocalFile(new Path("src/test/resources/links.txt"), joinFilePath);
        fs.copyFromLocalFile(new Path("src/test/resources/hadoopwiki.txt"), getInputDir());

        Job job = new ReduceSideJoin(createJobConf()).createJob(joinFilePath, new Path[] { getInputDir() },
                getOutputDir());
